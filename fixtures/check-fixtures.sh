...
run_case() {
  ...
  awk -F'\t' 'BEGIN{OFS=" "} !/^#/ {print $1, $2, $7, $9}' "$out_file" | sort -u >"$actual_file"

  while read -r id bulge_type direction bulge_size; do
    [[ -z "${id}" || "${id}" == \#* ]] && continue
    if ! grep -Fqx "${id} ${bulge_type} ${direction} ${bulge_size}" "$actual_file"; then
      echo "Fixture failure in ${case_dir}: missing ${id} ${bulge_type} ${direction} ${bulge_size}" >&2
      echo "Actual rows:" >&2
      cat "$actual_file" >&2
      return 1
    fi
